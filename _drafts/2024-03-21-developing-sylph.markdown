...
In this blog post, I will talk about the process of developing [sylph](https://github.com/bluenote-1577/sylph), a bioinformatics tool we [preprinted](https://www.biorxiv.org/content/10.1101/2023.11.20.567879v2) (with my advisor, [William](https://yunwilliamyu.net/content/)) recently and is now accepted-in-principle. 

I've always found the inner workings of how science is done fascinating. This is my contribution. My goal is to demystify tool development in bioinformatics a bit. 
...
* the _abundance_ of the organisms in your community. This is called metagenomic profiling, (e.g. Kraken)
* the  _similarity_ between an input genome and the genomes in your sample. This is called _containment_. 

See [this nice blog post](https://genomeinformatics.github.io/mash-screen/) for Mash screen for background.

This post won't be about sylph, but about its development process. Going forward, I'll assume some familiarity with computational metagenomics. 

#### Part 1 - k-mers, sketching, and Mash (prior to sylph)

I started working on sylph in December 2022. However, the first paper I ever read was the [Mash](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-016-0997-x) paper by Ondov et al. back in 2017 (on an undergraduate internship), which ended up being an inspiration for sylph. 

If you're unfamiliar with Mash, it's a method for comparing genomes by using a _sketch_ of k-mers (i.e. a set of k-length DNA substrings) to tell how similar two genomes are. The Mash algorithm is simple but powerful. It has tremendously influenced how I feel bioinformatics tools should be: __fast, easy to use, and accurate enough__. 

#### Part 2 - Initial motivation: let's write a faster Mash (late 2022)

Sylph started from a simple curiosity: when writing [skani](https://www.nature.com/articles/s41592-023-02018-3) in 2022, I noticed that I could create k-mer sketches faster than Mash. I adapted the k-mer processing routines from [minimap2](https://github.com/lh3/minimap2) and learned how to use SIMD to speed stuff up. This worked 3-4x faster than Mash. I thought this was neat. 

I care about metagenomics and microbiome informatics, and [sourmash](https://github.com/sourmash-bio/sourmash) is a similar tool which is more suitable for _metagenomics_ than Mash. I found that I could also sketch faster than sourmash (but they use a more comprehensive sketch, I think).

So I sought to write a stripped-down, multi-threaded sourmash for metagenomics primarily for personal use and also to possibly share -- I certainly didn't think of publishing anything at this point. This was built relatively quickly while I was zoning out during a Math conference I attended back home in Vancouver. 

#### Part 3 - Stumbling across Skmer (Sarmashghi et al., 2019)

When I was writing a paper about sequence alignment theory in mid-2022, I needed a citation for the intuition: "i.i.d Bernoulli mutation models work pretty well for k-mer statistics". So I went to the [following paper](https://doi.org/10.1089/cmb.2021.0431) by Blanca et al. _to see what they cited_, so I could learn something (and steal their citation). I found the paper ["Skmer: assembly-free and alignment-free sample identification using genome skims"](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-019-1632-4). 

I remember skimming (hah) through the paper, and while I didn't read through the math at the time, I understood the basic idea: **low-coverage sequencing biases sequence identity measurements when using k-mers**. It seemed like a great paper, but I didn't think more about it.
...
What I wanted was to compute the **containment average nucleotide identity** (ANI) accurately. This generalizes genome-genome ANI to _genome-metagenome_ ANI. And while there are methods for calculating ANI from k-mers, this issue obfuscates ANI calculation from k-mers.
...
So during 2022/2023 Christmas break, I read through the Skmer paper. I recall that I worked out the rough math for sylph at the McDonalds across the bridge near the airport in Vancouver... Anyway, the I implemented the statistical ANI correction method and it seemed to work on some simulated sequences -- success!
...
I had no way of dealing with this issue since sylph doesn't map reads -- it just checks for k-mers, of which many can be shared between different species. At this time, sylph's profiling didn't even work well at all. 

----

##### Back to sylph

In summary, I had a method that can only check if a genome is contained in a sample (quickly and more accurately than other methods), but **not compute abundances** (i.e., do profiling).

The funny thing is, I didn't try that hard to deal with the above issue. For some reason, I just accepted that this was not the "right" problem to solve for sylph -- could I really do something better than all of the profilers out there? 
...
Eventually, in April/May of 2023, I settled on just presenting sylph for containment ANI estimation. 
...
While preparing for manuscript submission, we worked a lot on figuring out how to present the paper. This was rather difficult given that people mostly just care about abundances, not containment ANI. Even now, most people only care about sylph as a profiler. Ultimately, I think the paper would have been fine, but not amazing.

#### Part 6 - Profiling: winner-take-all heuristic saves the day? (2023 Aug)

I was planning on submitting sylph in August 2023 (we had the cover letter ready), but I was randomly browsing through the sourmash/Mash GitHub one day -- I don't remember what for -- and I came across the "**winner-take-all**" heuristic somewhere. This is explained below. 
...
I didn't believe this would work, but I decided to give it a try. The idea was simple and it took less than an hour to implement. It worked surprisingly well, so I hit pause on the submission. 

Comparatively, the statistical model is a more interesting and important contribution, but this was the breakthrough... surprisingly.


#### Part 7 - Profiling successful! To preprinting (2023 Aug - 2023 Nov)

I finally had a profiler in August 2023. I did _not_ want to do the benchmarking necessary for developing a profiler. Unfortunately, the project seemed to have potential. 
...
I'm pleasantly surprised at sylph's reception so far. I've had people tell me personally/on social media that they've found sylph useful. Regardless of what journal we got into, as a tool developer, I've already been very happy with the outcome. Recently, I'm thrilled that sylph has been used for profiling **~2 million sequencing runs** in [Hunt et al.](https://www.biorxiv.org/content/10.1101/2024.03.08.584059v1)
...
These time proportions depend on the project. The underlying algorithm is *much simpler* than an assembler, which would take much more development time. Nevertheless, the following quote seems pertinent:
...
* If performance is not critical, author benchmarks don't matter that much. They'll just play around with it. 
* If performance is critical, you'll do your own tests instead of trusting author benchmarks, which always seem to place their tool at the top (mysteriously).

**Algorithm progress is highly non-linear as a function of time.** It's hard to predict progress. Furthermore, this project started out way different than how it ended up. I think a more natural approach to method development is correct rather than shooting for a goal, because it means you're tackling problems in the natural, *correct* way. 
